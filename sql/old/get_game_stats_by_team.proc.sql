DROP PROCEDURE IF EXISTS getGameStatsByTeam;

DELIMITER //
    CREATE PROCEDURE getGameStatsByTeam
        ()
    BEGIN
        ...
        UPDATE tempGameStat as a
            INNER JOIN 
                (SELECT c.GameID, c.TeamID, c.Points
                    FROM tempGameStat c
                    LEFT JOIN tempGameStat d ON c.Points < d.Points
                    WHERE d.GameID IS NULL) as b
